...
class TestUnitQuaternion:
    def test_constructor_variants(self):
        ...
        nt.assert_array_almost_equal(
            UnitQuaternion.Ry(90, "deg").vec, np.r_[1, 0, 1, 0] / math.sqrt(2)
        )
        nt.assert_array_almost_equal(
            UnitQuaternion.Ry(-90, "deg").vec, np.r_[1, 0, -1, 0] / math.sqrt(2)
        )
    ...
    def test_staticconstructors(self):
        ...
        for theta in np.r_[-pi / 2, 0, pi / 2, pi] * 180 / pi:
            nt.assert_array_almost_equal(
                UnitQuaternion.Rx(theta, "deg").R, rotx(theta, "deg")
            )

        for theta in [-pi / 2, 0, pi / 2, pi]:
            nt.assert_array_almost_equal(
                UnitQuaternion.Ry(theta, "deg").R, roty(theta, "deg")
            )
        ...
        nt.assert_array_almost_equal(UnitQuaternion.AngVec(-th, v).R, angvec2r(-th, v))
        nt.assert_array_almost_equal(
            UnitQuaternion.AngVec(-th, -v).R, angvec2r(-th, -v)
        )
        nt.assert_array_almost_equal(UnitQuaternion.AngVec(th, -v).R, angvec2r(th, -v))
        ...
        nt.assert_array_almost_equal(
            UnitQuaternion.EulerVec(-th * v).R, angvec2r(-th, v)
        )
    ...
    def test_multiply(self):
        ...
        # vector x vector
        qcompare(
            UnitQuaternion([ry, rz, rx]) * UnitQuaternion([rx, ry, rz]),
        ...
        # vector x scalar
        qcompare(
            UnitQuaternion([rx, ry, rz]) * ry,
            UnitQuaternion([rx * ry, ry * ry, rz * ry]),
        )
        ...
        # vector x scalar
        nt.assert_array_almost_equal(
            UnitQuaternion([ry, rz, rx]) * vy, np.c_[vy, -vx, vz]
        )
    ...
    def test_angle(self):
        ...
        for metric in range(5):
            assert uq1.angdist(other=uq1, metric=metric) == 0.0
            assert uq2.angdist(other=uq2, metric=metric) == 0.0
            assert uq1.angdist(other=uq2, metric=metric) == uq2.angdist(other=uq1, metric=metric)
    ...
    def test_miscellany(self):
        ...
        # angle
        # assert angle(u, u) == 0
        # assert angle(u, rx) == pi/4
        # assert angle(u, [rx, u]) == pi/4*np.r_[1, 0]
        # assert angle([rx, u], u) == pi/4*np.r_[1, 0]
        # assert angle([rx, u], [u, rx]) == pi/4*np.r_[1, 1]
        # TODO angle
    ...
    def test_interp(self):
        ...
        # assert length(q.interp(linspace(0,1, 10))) == 10
    ...
    def test_eq(self):
        ...
        nt.assert_array_almost_equal(
            UnitQuaternion([q1, q2, q3]) == UnitQuaternion([q1, q2, q3]),
            [True, True, True],
        )
        nt.assert_array_almost_equal(
            UnitQuaternion([q1, q1, q3]) == q1, [True, True, False]
        )
        nt.assert_array_almost_equal(
            q3 == UnitQuaternion([q1, q1, q3]), [False, False, True]
        )
...
class TestQuaternion:
    ...
    def test_equality(self):
        ...
        assert (qt1 == q1) == [True, True, False, False]
        assert (q1 == qt1) == [True, True, False, False]
        assert (qt1 == qt1) == [True, True, True, True]

        assert (qt2 == q1) == [True, False, False, True]
        assert (q1 == qt2) == [True, False, False, True]
        assert (qt1 == qt2) == [True, False, True, False]

        assert (qt1 != q1) == [False, False, True, True]
        assert (q1 != qt1) == [False, False, True, True]
        assert (qt1 != qt1) == [False, False, False, False]

        assert (qt2 != q1) == [False, True, True, False]
        assert (q1 != qt2) == [False, True, True, False]
        assert (qt1 != qt2) == [False, True, False, True]
    ...
    def test_miscellany(self):
        ...
        nt.assert_array_almost_equal(
            Quaternion([q, u, q]).norm(), [np.linalg.norm(v), 1, np.linalg.norm(v)]
        )
